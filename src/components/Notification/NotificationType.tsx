...
const NotificationType: React.FC<Props> = (props: Props) => {
  return (
    <div className="flex gap-3 px-2 sm:px-0">
      <button
        ...
        onClick={() => props.setIsRead(false)}
      >
        <InboxIcon className="h-4 w-4" />
        <div className={!props.isRead ? '' : `hidden sm:block`}>Inbox</div>
      </button>
      <button
        className={clsx(
          {
        ...
        onClick={() => props.setIsRead(true)}
      >
        <CheckCircleIcon className="h-4 w-4" />
        <div className={props.isRead ? '' : `hidden sm:block`}>Done</div>
      </button>
    </div>
  )
}
